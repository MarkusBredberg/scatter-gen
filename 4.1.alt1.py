...
for gen_model_name in gen_model_names:
    ...
    for fold, lr, reg, lambda_generate in param_combinations:
        ...
        if classifier in ['ScatterNet', 'ScatterDual']:
            ...
            # Compute (or load) the memmap cache for train scattering coefficients:
            train_cache_path = f"./.cache/train_scat_memmap_{galaxy_classes}_{fold}_{lambda_generate}_{dataset_portions[0]}_{FILTERED}_{num_galaxies}.npy"
            valid_cache_path = f"./.cache/valid_scat_memmap_{galaxy_classes}_{fold}_{lambda_generate}_{dataset_portions[0]}_{FILTERED}_{num_galaxies}.npy"
            # Cache scattering coefficients to a memmap file:
            ...
            # Create the dataset with the memmap:
