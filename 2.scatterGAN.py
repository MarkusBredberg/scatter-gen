...
REMOVEOUTLIERS = False # Sets dv to 'clean' if True and 'full' otherwise
...
def run_experiment(latent_dim, fold, lr_gen, lr_disc, gen_loss, disc_loss, adam_betas, weight_decay, label_smoothing):
    ...
    exp_name = f"latent{latent_dim}_cl{galaxy_class}_lrgen{lr_gen}_lrdisc{lr_disc}_gl{gen_loss}_dl{disc_loss}_ab{adam_betas}_wd{weight_decay}_ls{label_smoothing}_dv{data_version}"
    ...
    with logging_redirect_tqdm():
        for epoch in tqdm(range(1, epochs + 1), miniters=log_interval):
            ...
            for batch in train_loader:
                ...
                g_loss = gen_loss_fn(fake_pred_for_g)
                g_loss.backward()
                optimizer_G.step()
...
model.load_state_dict(torch.load(f"./GAN/generators/generator_{gan_type}_latent{latent_dim}_cl{11}_lrgen{lr_gen}_lrdisc{lr_disc}_gl{gen_loss}_dl{disc_loss}_ab{adam_betas}_wd{weight_decay}_ls{label_smoothing}_dv{data_version}_ep{500}_f{fold}.pth"))
model.eval()
with torch.no_grad():
    #fixed_noise = torch.randn(36, latent_dim, 1, 1, device=device)
...
gan_path = f"./GAN/generators/generator_{gan_type}_latent{latent_dim}_cl{11}_lrgen{lr_gen}_lrdisc{lr_disc}_gl{gen_loss}_dl{disc_loss}_ab{adam_betas}_wd{weight_decay}_ls{label_smoothing}_dv{data_version}_ep{500}_f{fold}.pth"
...
model.eval()
batch_array = [36, 100, 500, 1000, 5000]
...
plot_histograms(noise_generated_images_tensor, rep_generated, save_path=os.path.join(exp_dir, "rep_generated4_histograms.png"))

model.eval()
scale_array = [0.9, 0.95, 1, 1.05, 1.1]
for current_scale in scale_array:
    with torch.no_grad():
        generated_batch = generate_from_noise(
            model,
            latent_dim=latent_dim,
            ...
            scale=current_scale,
            DEVICE='cpu')
        save_path = os.path.join(exp_dir, f"rep_generated5_{current_scale}.png")
        save_images_tensorboard(generated_batch[:36], save_path, nrow=6)
...
generated_batch = generate_from_noise(
    ...
    num_samples=36,
    DEVICE='cpu')
save_path = os.path.join(exp_dir, f"rep_generated6.png")
save_images_tensorboard(generated_batch, save_path, nrow=6)
