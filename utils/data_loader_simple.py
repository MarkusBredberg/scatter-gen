...
def load_PSZ2(path=root_path + "PSZ2/classified/T100kpc", fold=5, BALANCE=False, AUGMENT=False, sample_size=300, target_classes=[53], crop_size=(1, 256, 256), downsample_size=(1, 256, 256), island=True, REMOVEOUTLIERS=True, train=False):
    ...
    if common:
        for h in common:
            print(f"Image with hash {h} appears in both classes, removing from train set.")
        # filter out any duplicated images from train set
        filtered_train = [(img, lbl) for img, lbl in zip(train_images, train_labels) if img_hash(img) not in common]
        train_images, train_labels = zip(*filtered_train) if filtered_train else ([], [])
        train_images, train_labels = list(train_images), list(train_labels)
        # filter out any duplicated images from eval set
        filtered_eval = [(img, lbl) for img, lbl in zip(eval_images, eval_labels) if img_hash(img) not in common]
        eval_images, eval_labels = zip(*filtered_eval) if filtered_eval else ([], [])
        eval_images, eval_labels = list(eval_images), list(eval_labels)
    ...
    #    for cls in target_classes:
    #        if kind == 'train':
    #            train_images_cls = [img for img, lbl in zip(train_images, train_labels) if lbl == cls]
    #            print("Length of train images for class", cls, ":", len(train_images_cls))
    #            np.save(f"/users/mbredber/scratch/data/PSZ2/classified/T100kpcSUB/DE/PSZ2_train_{cls}_{len(train_images_cls)}.npy", train_images_cls)
    #        else:
    #            eval_images_cls = [img for img, lbl in zip(eval_images, eval_labels) if lbl == cls]
    #            print("Length of eval images for class", cls, ":", len(eval_images_cls))
    #            np.save(f"/users/mbredber/scratch/data/PSZ2/classified/T100kpcSUB/DE/PSZ2_eval_{cls}_{len(eval_images_cls)}.npy", eval_images_cls)
...
def load_galaxies(galaxy_class, path=None, fold=None, island=None, crop_size=None, downsample_size=None, sample_size=None, REMOVEOUTLIERS=True, BALANCE=False, AUGMENT=False, train=None):
    """
    ...
    # Remove any overlap between train and test sets
    ...
    if common:
        for h in common:
            print(f"Image with hash {h} appears in both train and test sets; removing from both.")
        # filter train set
        filtered = [(img, lbl) for img, lbl in zip(train_images, train_labels) if img_hash(img) not in common]
        if filtered:
            train_images, train_labels = zip(*filtered)
        else:
            train_images, train_labels = [], []
        train_images, train_labels = list(train_images), list(train_labels)
        # filter test set
        filtered = [(img, lbl) for img, lbl in zip(eval_images, eval_labels) if img_hash(img) not in common]
        if filtered:
            eval_images, eval_labels = zip(*filtered)
        else:
            eval_images, eval_labels = [], []
        eval_images, eval_labels = list(eval_images), list(eval_labels)
    data = (train_images, train_labels, eval_images, eval_labels)


    # Zero-base labels for classifier compatibility
    train_images, train_labels, eval_images, eval_labels = data
    import torch
    ...
    train_labels = train_labels - min_lbl
    eval_labels = eval_labels - min_lbl
    return train_images, train_labels, eval_images, eval_labels