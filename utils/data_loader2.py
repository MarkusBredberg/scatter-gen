...
def load_PSZ2(path=root_path + "PSZ2/classified/", fold=5, sample_size=300, target_classes=[53], crop_size=(1, 256, 256), downsample_size=(1, 256, 256), GAUSSIAN_BLURRING=False, train=False):
    ...
    for cls in target_classes:
        ...
        for subfolder in [class_folder]:
            ...
            if CUBE:
                vf_list = version_folders[:num_versions]
                # find bases present in all vf_list folders
                sets = []
                for vf in vf_list:
                    folder = os.path.join(path, vf, subfolder)
                    names = {
                        fn[:-4]
                    ...
                    sets.append(names)
                base_names = set.intersection(*sets)
                ...
                version = "T100kpcSUB"
                folder = os.path.join(path, version, subfolder)
                for fn in sorted(os.listdir(folder)):
                    if not fn.lower().endswith(".npy"):
                        continue
                    npy_path = os.path.join(folder, fn)
                    arr = np.load(npy_path).astype(float).squeeze()
                    if arr.ndim == 3:
                        arr = arr.mean(axis=0)
                    if GAUSSIAN_BLURRING:
                    ...
                    raw = torch.from_numpy(arr).unsqueeze(0).float()  # [1,H,W]
                    tensor = apply_formatting(raw, crop_size, downsample_size)
                    images.append(tensor)           # shape [1, H, W]
                    labels.append(label)
                    filenames.append(fn[:-4])
...
def load_galaxies(galaxy_class, path=None, fold=None, island=None, crop_size=None, downsample_size=None, sample_size=None, REMOVEOUTLIERS=True, 
    ...
    if STRETCH and (len(crop_size) < 4 or len(downsample_size) < 4 or (crop_size[-4] == 1 and downsample_size[-4] == 1)):
        ...
        for class_id in train_class_ids:
            ...
            for img, suffix in [
                ...
                plt.savefig(f"./classifier/{class_id}_{source_name}_{suffix}_stretching.png", dpi=300)
                plt.close(fig)
        ...
        for class_id in eval_class_ids:
            ...
            clip_ranges_plot = [(0,100), (1,99), (60,99), (93,99)]
            funcs = [
                ('original', lambda x: x),
                ('log',      lambda x: log_stretch(x,  alpha=10)),
                ('asinh',    lambda x: asinh_stretch(x, alpha=10)),
    ...
    ##print("Shape of images before redistribution:", train_images.shape, eval_images.shape)
    # → DEBUG: what does our split look like now?
    from collections import Counter
    #print(f"Train size: {len(train_labels)},  Eval size: {len(eval_labels)}")
    #print("▶ Train class counts:", Counter(train_labels))
    #print("▶ Eval  class counts:", Counter(eval_labels))
    classes_present = torch.unique(torch.cat([torch.tensor(train_labels), torch.tensor(eval_labels)])).tolist()
    #print("Classes present in both sets:", classes_present)
    ...
    ##print("Shape of images after redistribution:", train_images.shape, eval_images.shape)
