...
from utils.classifiers import RustigeClassifier, ScatterSqueezeNet, MLPClassifier, DualClassifier, DANNClassifier, BinaryClassifier
...
print("Running script 4.1 try to reprode")
...
classifier = ["ProjectModel", "Rustige", "DANN", "ScatterNet", "ScatterSqueezeNet", "Binary"][-2]  # Choose one classifier model model
...
for gen_model_name in gen_model_names:
    ...
    if not TESTONGENERATED:
        ...
        if classifier in ['ProjectModel', 'Rustige', 'ScatterSqueezeNet', 'Binary']:
        ...
        if classifier in ['ScatterNet', 'ScatterSqueezeNet']:
        ...
        if classifier == 'ScatterSqueezeNet':
    ...
    for fold, lr, reg, lambda_generate in param_combinations:
        ...
        if classifier in ['ScatterNet', 'ScatterSqueezeNet']:
            ...
            if USE_MEMMAP:
                ...
                else: # classifier == "ScatterSqueezeNet"
        ...
        if classifier == "Rustige":
            ...
            models = {"ScatterSqueezeNet": {"model": DualClassifier(img_shape=tuple(valid_images.shape[1:]), scat_shape=scatdim, num_classes=num_classes).to(DEVICE)}}
