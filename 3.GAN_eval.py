...
galaxy_class, num_galaxies = 11, 1001028
...
if 'GAN' in model_names:
    ...
    print("Number of GAN models: ", len(gan_images))  # Should be equal to len(folds)
...
print("Number of VAE models: ", len(vae_model_names))
...
def calculate_total_intensities(real_images, generated_images_list, model_names):
    ...
    print("Length of all_generated_intensities ", len(all_generated_intensities))
...
def calculate_peak_intensities(real_images, generated_images_list, model_names):
    ...
    print("Length of all_generated_peaks ", len(all_generated_peaks))
...
def plot_asymmetry_vs_mse(original_images, constructed_images, encoder, save_path='./generator/eval/asymmetry_vs_mse_with_regression.png'):
...
def plot_images_by_asymmetry_bins(images, constructed_images, save_path='./generator/eval/asymmetry_bins.png'):
    start_time = time.time()
    # Step 1: Calculate asymmetry scores for each image
    asymmetry_scores = []
...
def plot_radial_intensity(
    ...
    save=True,
    save_path_generated='./generator/eval/radial_generated.png',
    save_path_reconstructed='./generator/eval/radial_reconstructed.png'
):
...
def plot_intensity_histograms_individually(
    ...
    save=True, save_path='./generator/eval/summedintensityhist.png', 
    ...
    for encoder in models_to_plot:
        ...
        ax1.step(bins, np.append(gen_hist, gen_hist[-1]), where='post', color=colors[encoder],
                 ...
                 label=f'{encoder} (n={len(concatenated_sums)}, RMAE={rmae_gen:.3f})' if RMAE else f'{encoder} (n={len(concatenated_sums)})',
    ...
    ax1.legend(loc='upper right' if 'peak' not in save_path else 'upper left', fontsize=16)
    ax1.grid(True)
...
def plot_latent_distributions_for_each_model(latent_representations, model_names, save=True, save_dir='./latent_distributions'):
    ...
    for i, latents in enumerate(latent_representations):
        ...
        plt.tight_layout()
        if save:
            model_save_path = os.path.join(save_dir, f'{model_names[i]}.png')
...
def plot_latent_cross_correlation(latent_representations, model_names, save_dir=f'./latent/'):
    ...
    for latents, model_name in zip(latent_representations, model_names):
            ...
            # Save the plot
            plot_save_path = os.path.join(save_dir, f'full_cross_correlation_{model_name}.png')
...
def plot_covariance(latent_vectors, num_dimensions=5, save=True, save_path='./covariance.png'):
    """
...
def old_plot_model_times_with_error_bars(models, save=True, save_path='./generator/eval/time_with_errors.png'):
...
def plot_image_grid(images, img_shape, nrow=10, ncol=10, title='Image grid', savepath=f'{save_dir}/image_grid.png', cmap='viridis'):
...
if not USE_GAN:
    vae_multmod(
        ...
        save=True,
        save_path=f"{save_dir}/reconstructed.png",
        show_title=False,
        show_originals=True
    )
...
plot_images_with_peaks(test_images, peaks_info_real, save_path=f'{save_dir}/peak_locations/real.png')
for i, encoder_name in enumerate(model_names):
    ...
    plot_asymmetry_score_histogram(test_images, num_bins=21, save_path=f"{save_dir}/asymmetry/histogram_{encoder_name}.png")
    plot_images_with_peaks(generated_images, peaks_info_gen, save_path=f'{save_dir}/peak_locations/generations_{encoder_name}.png')
    plot_image_grid(generated_images, img_shape, 10, 10, title="Generations", savepath=f"./generator/eval/{galaxy_class}_{num_galaxies}/{encoder_name}_generations.png")
    #plot_covariance(latent_representations[i], num_dimensions=5, save_path=f"{save_dir}/latent/{encoder_name}_covariance.png")
    ...
    if not USE_GAN and encoder_name not in ['GAN']:
        ...
        plot_asymmetry_vs_mse(test_images, reconstructed_images, encoder_name, save_path=f"{save_dir}/asymmetry/MSE_vs_asym_{encoder_name}.png")
        plot_images_by_asymmetry_bins(test_images, reconstructed_images, save_path=f'{save_dir}/asymmetry/reconstructions_{encoder_name}.png')
        plot_images_with_peaks(reconstructed_images, peaks_info_rec, save_path=f'{save_dir}/peak_locations/reconstructions_{encoder_name}.png')
...
if not USE_GAN:
    plot_peak_statistics(peaks_info_real, all_peak_intensity_stats_rec, vae_model_names, 
                     ...
                     save_path=f'{save_dir}/peak_statistics/values_recon.png')
plot_peak_statistics(peaks_info_real, all_peak_intensity_stats_gen, model_names, 
                     ...
                     save_path=f'{save_dir}/peak_statistics/values_gen.png')
...
if not USE_GAN:
    plot_peak_ratio_distribution(peaks_info_real, all_peak_intensity_stats_rec, vae_model_names, 
                                save_path=f'{save_dir}/peak_statistics/ratio_dist_recon.png')
plot_peak_ratio_distribution(peaks_info_real, all_peak_intensity_stats_gen, model_names, 
                             save_path=f'{save_dir}/peak_statistics/ratio_dist_gen.png')
...
if not USE_GAN:
    plot_peak_histograms_combined(peaks_info_real, all_peak_intensity_stats_rec, vae_model_names, prodtype='Reconstructed',
                        save_path=f'{save_dir}/peak_statistics/histograms_recon.png')
plot_peak_histograms_combined(peaks_info_real, all_peak_intensity_stats_gen, model_names, prodtype='Generated',
                     save_path=f'{save_dir}/peak_statistics/histograms_gen.png')
...
if not USE_GAN: 
    print("Shape of reconstructed images list: ", np.shape(reconstructed_images_list))
    print("Length of generated images:", len([img.shape for img in generated_images_list]))
    ...
    plot_intensity_histograms_individually(
        ...
        fixed_ylim=(9e-4, 1),
        save_path=f"{save_dir}/summedintensitydist_recon.png"
    )
...
plot_intensity_histograms_individually(
    ...
    fixed_xlim=(0, max_real+30),
    fixed_ylim=(9e-4, 1),
    save_path=f"{save_dir}/summedintensitydist_gen.png"
)
...
if not USE_GAN:
    ...
    plot_intensity_histograms_individually(
        ...
        fixed_xlim=(0, 1),
        fixed_ylim=(9e-4, 1),
        save_path=f"{save_dir}/peakintensitydist_recon.png"
    )
...
plot_intensity_histograms_individually(
    ...
    fixed_xlim=(0, 1),
    fixed_ylim=(9e-4, 1),
    save_path=f"{save_dir}/peakintensitydist_gen.png"
)
...
if not USE_GAN:
    ...
    plot_intensity_histograms_individually(
        ...
        fixed_xlim=(0, 1),
        fixed_ylim=(9e-9, 1),
        save_path=f"{save_dir}/totalintensitydist_recon.png"
    )
...
plot_intensity_histograms_individually(
    ...
    fixed_xlim=(0, 1),
    fixed_ylim=(9e-9, 1),
    save_path=f"{save_dir}/totalintensitydist_gen.png"
)
...
plot_radial_intensity(original_radial_intensity, models_rec_radial_intensity, models_gen_radial_intensity, model_names, title, save_path_generated=f"{save_dir}/radial_generated.png", save_path_reconstructed=f"{save_dir}/radial_reconstructed.png")
plot_stacked_images(test_images, reconstructed_images_list=None, generated_images_list=generated_images_list, model_names=model_names, save_path=f"{save_dir}/stacked_images.png")
...
if not USE_GAN:
    ...
    plot_latent_spaces(latent_representations, vae_model_names, save_path=f'{save_dir}/latent_spaces.png')
