...
class AlphaScattering2d_cov(object):
    ...
    def filters_tensor_morlet(self):
        ...
        if self.path is not None:
            ...
            Sihao_filters = FiltersSet(M=N, N=N, J=J, L=L)\
                .generate_wavelets(False, '', 'morlet')
        ...
        filt = np.zeros((J, 2*L, A, self.M, self.N), dtype=np.complex128)
    ...
    def filters_tensor_bump(self):
        ...
        if self.path is not None:
            ...
            fftphi = matfilters['filt_fftphi'].astype(np.complex128)  # (M,N) numpy
            hatphi = np.stack((np.real(fftphi), np.imag(fftphi)), axis=-1) # numpy
    
            fftpsi = matfilters['filt_fftpsi'].astype(np.complex128)  # (J,L2,M,N)
