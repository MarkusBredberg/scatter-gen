...
for lr, reg, ls, J_val, L_val, order_val, lo_val, hi_val, crop, down, vers in product(
    ...
    for gen_model_name in gen_model_names:
        ...
        if len(_out) == 4:
            ...
            raise ValueError(f"load_halos_and_relics(train=False) returned {len(_out)} values, expected 4 or 6")
        ...
        if _gen_versions and _anchor_versions:
            keep_set_test = {fn for fn in map(str, test_fns) if _has_anchors(fn, _anchor_versions)}
            test_images, test_labels, test_fns = _filter_to_keep_set(test_images, test_labels, test_fns, keep_set_test)
            print(f"[TEST-anchor] kept {len(test_fns)} after gating by {_anchor_versions}")
            ...
            assert info_test['removed_total'] == 0, "After anchor gating there should be no rt* drops."
        ...
        for fold, lr, reg, lambda_generate in param_combinations:
            ...
            if TRAINONGENERATED:
                ...
                if len(_out) == 4:
                    ...
                    if _gen_versions and _anchor_versions:
                        keep_train = {fn for fn in map(str, train_fns) if _has_anchors(fn, _anchor_versions)}
                        keep_valid = {fn for fn in map(str, valid_fns) if _has_anchors(fn, _anchor_versions)}
                        train_images, train_labels, train_fns = _filter_to_keep_set(train_images, train_labels, train_fns, keep_train)
                        valid_images, valid_labels, valid_fns = _filter_to_keep_set(valid_images, valid_labels, valid_fns, keep_valid)
                        print(f"[TRAIN-anchor] kept {len(train_fns)}; [VALID-anchor] kept {len(valid_fns)}")
                        ...
                        assert info_tr['removed_total'] == 0, "After anchor gating there should be no rt* drops (train)."
                        assert info_va['removed_total'] == 0, "After anchor gating there should be no rt* drops (valid)."
                        
                        # ---- Train/Test leakage check (AFTER train_images and test_images are defined) ----
                        import hashlib
                        def img_hash(img: torch.Tensor) -> str:
                            arr = img.cpu().contiguous().numpy()
                            return hashlib.sha1(arr.tobytes()).hexdigest()

                        train_hashes = {img_hash(img) for img in train_images}   # ok if 4D or 5D; iterates per-sample
                        test_hashes  = {img_hash(img) for img in test_images}
                        overlap = train_hashes & test_hashes
                        assert not overlap, f"Overlap detected: {len(overlap)} images appear in both train and test!"
                        # -------------------------------------------------------------------------------
